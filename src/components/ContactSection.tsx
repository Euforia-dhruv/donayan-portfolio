    ...
    <section id="contact" className="py-24 md:py-36 bg-cinema-black border-t border-cinema-white/8">
      <div className="max-w-[1400px] mx-auto px-8 md:px-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
          <div>
            <h2 className="text-heading md:text-heading-lg font-switzer font-[300] text-cinema-white leading-[1] tracking-[-0.03em] text-balance">{siteContent.contact.header}</h2>
            <p className="mt-6 text-subheading font-switzer font-[300] text-stone leading-[1.4] max-w-md">{siteContent.contact.intro}</p>
          ...
          <div id="contact-form">
            {submitted ? (
              ...
              <form action="https://formspree.io/f/your-form-id" method="POST" onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label htmlFor="name" className="block text-caption font-switzer font-[400] text-stone uppercase tracking-[0.02em] mb-2">Name</label>
                  <input type="text" id="name" name="name" required
                    className="w-full px-5 py-3.5 bg-transparent border border-cinema-white/10 text-cinema-white text-body-sm font-switzer font-[300] focus:outline-none focus:border-cinema-white/30 transition-colors"
                    placeholder="Your name" />
                </div>
                <div>
                  <label htmlFor="email" className="block text-caption font-switzer font-[400] text-stone uppercase tracking-[0.02em] mb-2">Email</label>
                  <input type="email" id="email" name="email" required
                    className="w-full px-5 py-3.5 bg-transparent border border-cinema-white/10 text-cinema-white text-body-sm font-switzer font-[300] focus:outline-none focus:border-cinema-white/30 transition-colors"
                    placeholder="your@email.com" />
                </div>
                <div>
                  <label htmlFor="message" className="block text-caption font-switzer font-[400] text-stone uppercase tracking-[0.02em] mb-2">Message</label>
                  <textarea id="message" name="message" required rows={4}
                    className="w-full px-5 py-3.5 bg-transparent border border-cinema-white/10 text-cinema-white text-body-sm font-switzer font-[300] focus:outline-none focus:border-cinema-white/30 transition-colors resize-none"
                    placeholder="Tell me about your project..." />
                </div>
                {error && <p className="text-body-sm font-switzer text-stone">{siteContent.contact.error}</p>}
                <button type="submit"
                  className="inline-flex items-center gap-2 px-8 py-3.5 bg-gold text-cinema-black text-body-sm font-switzer font-[400] uppercase tracking-[0.02em] hover:opacity-85 transition-all duration-300 cursor-pointer border-none"
                  style={{ borderRadius: "1440px" }}>
