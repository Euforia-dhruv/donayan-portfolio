...
export default function VideoLightbox({ url, type, title, onClose }: VideoLightboxProps) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-ink-black/95 backdrop-blur-md" onClick={onClose}>
      <div className="relative w-full max-w-5xl mx-4 md:mx-8" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute -top-12 right-0 text-bone-white/60 hover:text-bone-white text-caption font-switzer uppercase tracking-[0.02em] bg-transparent border-none cursor-pointer transition-colors">Close [ESC]</button>

        <div className="aspect-video bg-bone-white overflow-hidden border border-ash/30">
          {type === "youtube" ? (
            ...
            <div className="w-full h-full flex items-center justify-center flex-col gap-4">
              <p className="text-body font-switzer font-[300] text-graphite">Instagram content</p>
              <a href={url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-2.5 bg-ink-black text-bone-white text-caption font-switzer font-[400] uppercase tracking-[0.02em] no-underline" style={{ borderRadius: "1440px" }}>Open in Instagram<svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M3 9l6-6M9 3H5M9 3v4" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" /></svg></a>
            </div>
          )}
        </div>

        <p className="mt-4 text-body-sm font-switzer font-[400] text-graphite">{title}</p>
      </div>
    </div>
  );
}
