    ...
    >
      <div className="relative w-full overflow-hidden" style={{ aspectRatio: item.aspect }}>
        ...
        {item.type === "document" && !item.thumbnail && (
          <div className="absolute inset-0 flex items-center justify-center bg-ash/10">
            <div className="text-center">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="mx-auto text-graphite mb-2">
              ...
              <p className="text-caption font-switzer font-[400] text-graphite uppercase tracking-[0.02em]">{item.brand}</p>
            </div>
          </div>
        )}
        {isVideoCard && !isHovered && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-bone-white/80 flex items-center justify-center transition-all duration-300 group-hover:bg-bone-white group-hover:scale-110">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" className="ml-0.5"><path d="M8 5v14l11-7L8 5z" fill="#000000" /></svg>
            </div>
          </div>
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-ink-black/60 via-ink-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-400 flex flex-col justify-end p-4 md:p-5"
          ...
          <div className="transform translate-y-3 group-hover:translate-y-0 transition-transform duration-400" style={{ transitionTimingFunction: "cubic-bezier(0.25,0.46,0.45,0.94)" }}>
            <div className="flex flex-wrap items-center gap-2 mb-1.5">
              <span className="text-caption font-switzer font-[400] text-bone-white/70 uppercase tracking-[0.02em]">{item.category}</span>
              <span className="text-bone-white/40">·</span>
              <span className="text-caption font-switzer font-[400] text-bone-white/40 uppercase tracking-[0.02em]">{item.year}</span>
            </div>
            <p className="text-body font-switzer font-[300] text-bone-white leading-[1.2]">{item.brand}</p>
            <p className="text-caption font-switzer font-[400] text-bone-white/50 mt-0.5 uppercase tracking-[0.02em]">{item.role}</p>
            <p className="text-caption font-switzer font-[500] text-bone-white/80 mt-2 uppercase tracking-[0.02em] opacity-0 group-hover:opacity-100 transition-opacity duration-300">View {isVideoCard ? "Project" : "Case Study"} →</p>
          </div>
        </div>
        {isVideoCard && (
          <div className="absolute top-3 left-3 px-2 py-0.5 text-caption font-switzer font-[400] uppercase tracking-[0.02em] bg-bone-white/80 text-graphite">{getPlatformLabel(item.src)}</div>
        )}
        {isVideoCard && (
          <div className="absolute top-3 right-3 px-2 py-0.5 text-caption font-switzer font-[400] uppercase tracking-[0.02em] bg-bone-white/80 text-graphite">{getDurationLabel(item.src)}</div>
        )}
      </div>
    </div>
  );
...
export default function ShowcaseWall() {
  ...
  return (
    <>
      <section className="py-20 md:py-28 bg-bone-white border-t border-ash/50">
        <div className="max-w-[1440px] mx-auto px-5 md:px-8">
          <div className="mb-12 md:mb-16">
            <p className="text-caption font-switzer font-[400] text-graphite uppercase tracking-[0.02em]">The Archive</p>
            <h2 className="text-heading md:text-heading-lg font-switzer font-[300] text-ink-black leading-[1] tracking-[-0.03em] mt-2">Showcase</h2>
          </div>
      ...
      {video && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-ink-black/95 backdrop-blur-xl" onClick={handleClose}>
          <div className="relative w-full max-w-5xl mx-4" onClick={(e) => e.stopPropagation()}>
            <button onClick={handleClose} className="absolute -top-12 right-0 text-bone-white/60 hover:text-bone-white text-caption font-switzer uppercase tracking-[0.02em] bg-transparent border-none cursor-pointer transition-colors z-10">Close [ESC]</button>
            <p className="text-body-sm font-switzer font-[400] text-bone-white/50 mb-3 truncate pr-20">{video.title}</p>
            <div className="relative aspect-video bg-bone-white overflow-hidden">
              {isEmbeddable(video.url) ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <div className="text-center">
                    <p className="text-body-sm font-switzer font-[400] text-graphite mb-4">Open on Instagram to view</p>
                    <a href={video.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-2.5 bg-ink-black text-bone-white text-caption font-switzer font-[400] uppercase tracking-[0.02em] no-underline" style={{ borderRadius: "1440px" }}>Open on Instagram</a>
                  </div>
                </div>
              )}
            </div>
